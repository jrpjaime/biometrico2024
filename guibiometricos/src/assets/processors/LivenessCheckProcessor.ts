...
import { Userlife } from "src/app/core/model/userlife";
...
export class LivenessCheckProcessor implements FaceTecFaceScanProcessor {
  ...
  public processSessionResultWhileFaceTecSDKWaits(sessionResult: FaceTecSessionResult, faceScanResultCallback: FaceTecFaceScanResultCallback) {
    ...
    this.latestNetworkRequest.open("POST", Config.BaseURL + "?op=liveness-3d");
    ...
    this.latestNetworkRequest.setRequestHeader("X-Token-Authentication", Config.xTokenAuthentication);
      ...
      if(this.latestNetworkRequest.readyState === XMLHttpRequest.DONE) {
        try {
          ...
          if(responseJSON.wasProcessed) {
            ...
            console.log("responseJSON.isSuccess "+ responseJSON.isSuccess);
            sessionStorage.setItem ("life", encodeURIComponent ( JSON.stringify (true) ) );

            console.log("auditTrailImage:    "+parameters.auditTrailImage);
            ...
            sessionStorage.setItem ( "_uSeR_sEssIoN_liFe_", encodeURIComponent ( JSON.stringify ( userlife ) ) );


            console.log("sessionResult.sessionId: "+sessionResult.sessionId);

            setTimeout(() => {
                window.location.href = "http://localhost:4200/validacion/credencial";
              }, 1000);
